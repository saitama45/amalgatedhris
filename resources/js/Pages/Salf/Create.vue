...
const canManageSalf = hasPermission('salf.create');
...
const form = useForm({
    employee_id: props.currentUserEmployee?.id || '',
    department_id: props.currentUserEmployee?.active_employment_record?.department_id || props.currentUserEmployee?.department_id || '',
    company_id: props.currentUserEmployee?.active_employment_record?.company_id || props.currentUserEmployee?.company_id || '',
    period_covered: '',
    approved_by: '',
    status: 'draft',
    items: [
        { is_header: false, section: '', area_of_concern: '', action_plan: '', support_group: '', target_date: '', actual_value: 0, target_value: 100, remarks: '' }
    ]
});
...
const submit = () => {
    form.post(route('salf.store'));
};
</script>

<template>
    <Head title="Create New SALF - HRIS" />
    <AppLayout>
        <template #header>
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="font-bold text-2xl text-slate-800 leading-tight">Create Strategic Action Layout Form</h2>
                    <p class="text-sm text-slate-500 mt-1">Design and plan your strategic goals.</p>
                </div>
            </div>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <form @submit.prevent="submit">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 mb-6 overflow-hidden relative">
                         ...
                         <div class="absolute bottom-0 left-0 w-64 h-64 bg-indigo-50/50 rounded-full -ml-32 -mb-32 blur-3xl"></div>
                        ...
                        <div class="relative grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                            ...
                            <div>
                                ...
                                <input v-model="form.period_covered" type="text" placeholder="e.g. Jan 26 - Feb 10, 2026" class="w-full bg-slate-50 border-slate-200 rounded-xl focus:ring-blue-600 focus:border-blue-600 transition-all text-sm font-semibold" />
                            ...
                            <div>
                                ...
                                <input v-model="form.approved_by" type="text" placeholder="Approver Name" class="w-full bg-slate-50 border-slate-200 rounded-xl focus:ring-blue-600 focus:border-blue-600 transition-all text-sm font-semibold" />
